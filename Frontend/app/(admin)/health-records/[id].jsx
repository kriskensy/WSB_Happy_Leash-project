...
export default function HealthRecordDetails() {
  ...
  const toggleVaccinationStatus = async () => {
    try {
      setLoading(true);
      const token = await AsyncStorage.getItem("userToken");
      const response = await fetch(`http://10.0.2.2:5000/api/HealthRecord/${id}/vaccination`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        setHealthRecord(prev => ({ ...prev, vaccinationStatus: !prev.vaccinationStatus }));
      } else {
    ...
    <ScrollView style={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.detailValue}>{formatDate(healthRecord.recordDate)}</Text>
          
          <Text style={styles.detailLabel}>Description:</Text>
          <Text style={styles.detailValue}>{healthRecord.description || "No description"}</Text>
          
          <Text style={styles.detailLabel}>Vaccination Status:</Text>
          <Text style={[
            styles.detailValue,
            { color: healthRecord.vaccinationStatus ? COLORS.success : COLORS.warning }
          ]}>
        ...
        <TouchableOpacity
          style={[styles.button, { 
            backgroundColor: healthRecord.vaccinationStatus ? COLORS.warning : COLORS.success 
          }]}
          onPress={toggleVaccinationStatus}
        >
          <Text style={styles.buttonText}>
            {healthRecord.vaccinationStatus ? "Mark as Not Vaccinated" : "Mark as Vaccinated"}
          </Text>
        </TouchableOpacity>
        ...
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity
            style={[styles.button, { flex: 1, marginLeft: 5, backgroundColor: COLORS.danger }]}
