...
export default function UserDetails() {
  ...
  const fetchUser = async () => {
    try {
      ...
      const response = await fetch(`http://10.0.2.2:5000/api/User/${id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <ScrollView style={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        {user.profilePictureURL ? <Image source={{ uri: user.profilePictureURL }} style={{ width: 120, height: 120, borderRadius: 60, marginBottom: 10 }} /> : null}
        ...
        <View style={styles.buttonRow}>
          <TouchableOpacity style={[styles.button, { flex: 1, marginRight: 5 }]} onPress={() => router.push(`/users/edit/${id}`)}>
        ...
        <TouchableOpacity style={[styles.button, { backgroundColor: COLORS.secondary, marginTop: 10 }]} onPress={() => router.push("/users")}>
