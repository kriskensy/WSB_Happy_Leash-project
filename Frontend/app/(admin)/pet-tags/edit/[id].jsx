  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const petsRes = await fetch("http://10.0.2.2:5000/api/Pet", { headers: { Authorization: `Bearer ${token}` } });
        const tagsRes = await fetch("http://10.0.2.2:5000/api/Tag", { headers: { Authorization: `Bearer ${token}` } });
        const petTagRes = await fetch(`http://10.0.2.2:5000/api/PetTag/${id}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    <ScrollView style={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        <View style={styles.pickerContainer}>
          <Picker selectedValue={petId} onValueChange={setPetId} style={styles.picker}>
            {pets.map(pet => (
        ...
        <View style={styles.pickerContainer}>
          <Picker selectedValue={tagId} onValueChange={setTagId} style={styles.picker}>
            {tags.map(tag => (
