...
export default function PetDetails() {
  ...
  const fetchPet = async () => {
    try {
      ...
      const response = await fetch(`http://10.0.2.2:5000/api/Pet/${id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <ScrollView style={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        {pet.imageUrl ? <Image source={{ uri: pet.imageUrl }} style={{ width: 150, height: 150, borderRadius: 12, marginBottom: 10 }} /> : null}
        <View style={styles.card}>
          ...
          <Text style={styles.detailValue}>{pet.description || "No description"}</Text>
          <Text style={styles.detailLabel}>Adopted:</Text>
          <Text style={styles.detailValue}>{pet.adopted ? "Yes" : "No"}</Text>
        </View>
        <View style={styles.buttonRow}>
          <TouchableOpacity style={[styles.button, { flex: 1, marginRight: 5 }]} onPress={() => router.push(`/pets/edit/${id}`)}>
            <Text style={styles.buttonText}>Edit</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.button, { flex: 1, marginLeft: 5, backgroundColor: COLORS.danger }]} onPress={confirmDelete}>
            <Text style={styles.buttonText}>Delete</Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity style={[styles.button, { backgroundColor: COLORS.secondary, marginTop: 10 }]} onPress={() => router.push("/pets")}>
