  ...
  const confirmDelete = (id, petName, userName) => {
    Alert.alert(
      ...
      [
        ...
        { text: "Delete", onPress: () => handleDeleteAdoptionRequest(id), style: "destructive" }
      ]
    );
  };
        ...
        renderItem={({ item }) => (
          <View style={styles.listItemWithActions}>
            <ListItem 
              ...
              subtitle={`Status: ${item.isApproved ? 'Approved' : 'Pending'} - ${formatDate(item.requestDate)}`}
              onPress={() => router.push(`/adoption-requests/${item.id}`)}
            />
            <View style={styles.actionButtons}>
              <TouchableOpacity
                style={styles.actionButton}
                onPress={() => router.push(`/adoption-requests/edit/${item.id}`)}
              >
                <Ionicons name="pencil" size={20} color={COLORS.primary} />
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.actionButton}
                onPress={() => confirmDelete(item.id, item.petName, item.userName)}
              >
