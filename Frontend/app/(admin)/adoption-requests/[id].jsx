...
export default function AdoptionRequestDetails() {
  ...
  const fetchAdoptionRequest = async () => {
    try {
      ...
      const response = await fetch(`http://10.0.2.2:5000/api/AdoptionRequest/${id}`, {
        headers: {
  ...
  const handleApproveReject = async (isApproved) => {
    try {
      setLoading(true);
      const token = await AsyncStorage.getItem("userToken");
      const response = await fetch(`http://10.0.2.2:5000/api/AdoptionRequest/${id}/status`, {
        method: "PUT",
        headers: {
    ...
    <ScrollView style={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.detailValue}>{`${adoptionRequest.userFirstName} ${adoptionRequest.userLastName}`}</Text>
          
          <Text style={styles.detailLabel}>Message:</Text>
          <Text style={styles.detailValue}>{adoptionRequest.message || "No message provided"}</Text>
          
          <Text style={styles.detailLabel}>Request Date:</Text>
          <Text style={styles.detailValue}>{formatDate(adoptionRequest.requestDate)}</Text>
