  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const adoptionRequestResponse = await fetch(`http://10.0.2.2:5000/api/AdoptionRequest/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert("Success", "Adoption request updated successfully", [
          { text: "OK", onPress: () => router.push(`/adoption-requests/${id}`) }
        ]);
      } else {
        const errorData = await response.json();
        Alert.alert("Error", errorData.message || "Failed to update adoption request");
      }
    } catch (error) {
